    ...
    return (
        <div className="cart-list">
            ...
            {items.length === 0 ? (
                ...
                items.map((item) => <CartItem key={item.id} id={item.id} title={item.title} image={item.image} price={item.price} quantity={item.quantity} handleDeleteCart={handleDeleteCart} />)
            )}
